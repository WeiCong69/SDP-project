...
<?php 
    ...
    $data="SELECT A.JobID,A.JobTitle,B.status from job as A INNER JOIN payment as B on A.JobID=B.JobID where A.Job_Status='Completed' AND B.Status='Pending'";
    ...
    if(isset($_POST['check'])){
        ...
        $id=$_POST['check'];
        $data2="SELECT C.Full_name,A.JobID,C.`Part-timerID`,D.PaymentID from job as A INNER JOIN applicationid as B on A.JobID=B.JobID 
        inner JOIN `part-timer` as C on B.`Part-timerID`=C.`Part-timerID` INNER JOIN payment as D on D.JobID=A.JobID
        where A.Job_Status='Completed' AND D.Status='Pending' AND A.JobID='$id'";
        if(!$result2=mysqli_query($con,$data2)){
        die('Error'.mysqli_error($con));
        }      
    }
...
?>
  <div class="column">
    <div class="card">
      ...
      <a href="PaymentAndRating.php?JobID_fixed=<?php echo $r['JobID'];?>" class="button">CHECK</a>
    </div>
  </div>
